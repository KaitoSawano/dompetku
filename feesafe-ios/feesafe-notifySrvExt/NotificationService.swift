...
class NotificationService: UNNotificationServiceExtension {
	...
	private var feesafeStarted: Bool = false
	private var srvExtDone: Bool = false
	
	private var business: FeeSafeBusiness? = nil
	...
	private func startConnectionTimer() {
		...
		guard let groupPrefs = FeeSafeManager.shared.groupPrefs() else {
	...
	private func didReceiveXpcMessage(_ msg: XpcMessage) {
		...
		if msg == .available {
			...
			// The main feesafe app is running.
			log.info("didReceiveXpcMessage: available (main feesafe app is running)")
	...
	private func startFeeSafe() {
		log.trace(#function)
		assertMainThread()
		
		guard !srvExtDone else {
			log.info("startFeeSafe(): ignoring: srvExtDone")
			return
		}
		guard !feesafeStarted else {
			log.warning("startFeeSafe(): ignoring: already started")
			return
		}
		feesafeStarted = true
		
		let newBusiness = FeeSafeManager.shared.setupBusiness(targetNodeIdHash)
	...
	private func stopFeeSafe() {
		log.trace(#function)
		assertMainThread()
		
		guard feesafeStarted else {
			log.info("stopFeeSafe(): ignoring: already stopped")
			return
		}
		feesafeStarted = false
		
		FeeSafeManager.shared.teardownBusiness()
		business = nil
	}
	...
	private func finish() {
		...
		// and to do that we call FeeSafeManager.groupPrefs().
	...
	private func updateNotificationContent_receivedPayment(
		...
		let groupPrefs = FeeSafeManager.shared.groupPrefs()
	...
	private func updateNotificationContent_pendingSettlement(
		...
		content.title = String(localized: "Please start FeeSafe", comment: "")
	...
	private func formatAmount(_ groupPrefs: GroupPrefs_Wallet, msat: Int64) -> String {
		...
		let exchangeRate = FeeSafeManager.shared.exchangeRate(fiatCurrency: fiatCurrency)
