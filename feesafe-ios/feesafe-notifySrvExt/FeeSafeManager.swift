...
fileprivate let filename = "FeeSafeManager"
#if DEBUG && true
 ...
 * # Architecture notes:
 *
 * In a previous implementation, we created the `FeeSafeBusiness` instance once,
 ...
 */
class FeeSafeManager {
	
	public static let shared = FeeSafeManager()
	
	private static let feesafeGlobal = FeeSafeGlobal(ctx: PlatformContext.default)
	
	private var business: FeeSafeBusiness? = nil
	private var oldBusiness: FeeSafeBusiness? = nil
	...
	public func setupBusiness(_ target: String?) -> FeeSafeBusiness {
		log.trace(#function)
		assertMainThread()
		...
		let newBusiness = FeeSafeBusiness(feesafeGlobal: FeeSafeManager.feesafeGlobal)

		newBusiness.feesafeGlobal.networkMonitor.disable()
		newBusiness.feesafeGlobal.currencyManager.disableAutoRefresh()
	...
	private func unlock(_ recoveryPhrase: RecoveryPhrase?, _ targetNodeIdHash: String?) {
		...
		business.feesafeGlobal.currencyManager.refreshAll(targets: [primaryFiatCurrency], force: false)
		Task { @MainActor [business, weak self] in
			for await rates in business.feesafeGlobal.currencyManager.ratesSequence() {
