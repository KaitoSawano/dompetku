...
fileprivate let filename = "KotlinPublishers+FeeSafe"
#if DEBUG && true
...
// MARK: -
extension FeeSafeShared.NotificationsManager {

	struct NotificationItem: Identifiable {
		let ids: Set<Lightning_kmpUUID>
		let notification: FeeSafeShared.Notification
			...
			.map { originalArray in
				let transformedArray: [NotificationItem] = originalArray.compactMap { value in
					guard
						...
						let notification = pair.second as? FeeSafeShared.Notification
					else {
						return nil
					}
