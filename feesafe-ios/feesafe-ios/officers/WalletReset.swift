...
/// In particular, the vast majority of the UI is deeply connected with the FeeSafeShared instance.
/// And we're trying to reset FeeSafeShared instance, along with the UI.
...
///    ensuring that no references to the previous FeeSafeShared instance are being used.
///
class WalletReset {
	...
	*/
	...
	/**
	 * The next step is to shutdown the feesafe-shared layer (i.e. the Kotlin Multiplatform stuff).
	...
	private func step3() {
		...
		guard
			let groupDir = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.co.acinq.feesafe")
		else {
			return step4()
		}
		...
		let chainName: String = Biz.business.chain.feesafeName
