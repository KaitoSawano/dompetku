...
/// Short-hand for `BusinessManager.feesafeGlobal`
///
var BizGlobal: FeeSafeGlobal {
	return BusinessManager.feesafeGlobal
}
...
/// Manages the `FeeSafeBusiness` instance, which is the shared logic written in Kotlin Multiplatform.
///
class BusinessManager {

	public static let feesafeGlobal = FeeSafeGlobal(ctx: PlatformContext.default)
	...
	///
	public let business: FeeSafeBusiness
	...
	init() {
		...
		business = FeeSafeBusiness(feesafeGlobal: BusinessManager.feesafeGlobal)
	...
	private func setup(_ walletId: WalletIdentifier) {
		...
		Task { @MainActor [self] in
			...
			for await newInfo in peer.feesafeSwapInWallet.swapInAddressSequence() {
				if let newInfo {
