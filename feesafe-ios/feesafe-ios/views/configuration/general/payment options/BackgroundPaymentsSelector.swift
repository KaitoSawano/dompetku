	...
	func section_enabled() -> some View {
		...
		Section {
			VStack(alignment: HorizontalAlignment.leading, spacing: 0) {
				...
				Label {
					Text("(even if FeeSafe isn't running)").foregroundColor(.secondary)
				} icon: {
					invisibleImage()
				}
				.font(.callout)
	...
	func section_disabled() -> some View {
		...
		Section {
			VStack(alignment: HorizontalAlignment.leading, spacing: 0) {
				...
				Label {
					Text("You can only receive payments if FeeSafe is running in the foreground.")
						.font(.callout)
				} icon: {
				...
				Label {
					VStack(alignment: HorizontalAlignment.leading, spacing: 8) {
						...
						Text("(even if FeeSafe isn't running)")
					...
					VStack(alignment: HorizontalAlignment.leading, spacing: 10) {
						...
						HStack(alignment: VerticalAlignment.top, spacing: 8) {
							...
							Button {
								...
								Text("Open iOS Settings for FeeSafe")
							}
						}
						HStack(alignment: VerticalAlignment.top, spacing: 8) {
							Text(verbatim: "*").bold()
