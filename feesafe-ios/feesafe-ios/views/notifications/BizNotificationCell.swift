...
struct BizNotificationCell: View {
	...
	let item: FeeSafeShared.NotificationsManager.NotificationItem
	...
	@ViewBuilder
	var body: some View {
		if let reason = item.notification as? FeeSafeShared.Notification.PaymentRejected {

			if let reason = reason as? FeeSafeShared.Notification.PaymentRejected.OverAbsoluteFee {
				body_paymentRejected_overFee(Either.Left(reason))
				
			} else if let reason = reason as? FeeSafeShared.Notification.PaymentRejected.OverRelativeFee {
		...
		} else if let reason = item.notification as? FeeSafeShared.WatchTowerOutcome {

			if let reason = reason as? FeeSafeShared.WatchTowerOutcome.Nominal {
				body_watchTowerSuccess(reason)

			} else if let reason = reason as? FeeSafeShared.WatchTowerOutcome.RevokedFound {
				body_watchTowerRevoked(reason)

			} else if let reason = reason as? FeeSafeShared.WatchTowerOutcome.Unknown {
	...
	func body_paymentRejected_overFee(
		_ either: Either<
			FeeSafeShared.Notification.PaymentRejected.OverAbsoluteFee,
			FeeSafeShared.Notification.PaymentRejected.OverRelativeFee
		>
	) -> some View {
	...
	func body_paymentRejected(
		_ reason: FeeSafeShared.Notification.PaymentRejected
	) -> some View {
	...
	func body_watchTowerSuccess(
		_ reason: FeeSafeShared.WatchTowerOutcome.Nominal
	) -> some View {
	...
	func body_watchTowerRevoked(
		_ reason: FeeSafeShared.WatchTowerOutcome.RevokedFound
	) -> some View {
	...
	func body_watchTowerUnknown(
		_ reason: FeeSafeShared.WatchTowerOutcome.Unknown
	) -> some View {
