...
struct NotificationsView : View {
	...
	@State var bizNotifications_payment: [FeeSafeShared.NotificationsManager.NotificationItem] = []
	@State var bizNotifications_watchtower: [FeeSafeShared.NotificationsManager.NotificationItem] = []
	...
	func bizNotificationsChanged(_ list: [FeeSafeShared.NotificationsManager.NotificationItem]) {
		log.trace("bizNotificationsChanges()")
		
		bizNotifications_payment = list.filter({ item in
			if let _ = item.notification as? FeeSafeShared.Notification.PaymentRejected {
				return true
			} else {
				return false
			}
		...
		bizNotifications_watchtower = list.filter({ item in
			if let watchTower = item.notification as? FeeSafeShared.WatchTowerOutcome {
				// Remove "Nominal" notifications (which just mean everything is working as expected)
				return !(watchTower is FeeSafeShared.WatchTowerOutcome.Nominal)
			} else {
				return false
			}
		})
	...
	func openMempoolFullURL() {
		...
		if let url = URL(string: "https://feesafe.acinq.co/faq#high-mempool-size-impacts") {
			openURL(url)
		}
	}
	...
	func deleteNotification(_ item: FeeSafeShared.NotificationsManager.NotificationItem) {
		log.trace(#function)
